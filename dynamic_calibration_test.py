...
def run_tests():
    ...
    assert res1["status"] == "success", "Test Case 1 failed: status should be success"
    assert len(res1["correction_factors_db"]) == 8, "Should have 8 correction factors"
    # Sony WH-1000XM4 mock response: [1.5, 0.8, -0.2, -1.0, -2.0, -3.5, -2.5, -1.5]
    # Google Pixel Buds Pro mock response: [-0.2, 0.2, 0.5, 1.5, 0.8, -1.0, -2.0, -3.0]
    # Raw correction = Baseline - User
    # 250Hz: -1.7, 500Hz: -0.6, 1000Hz: 0.7, 2000Hz: 2.5, 3000Hz: 2.8, 4000Hz: 2.5, 6000Hz: 0.5, 8000Hz: -1.5
    expected_raw = [-1.7, -0.6, 0.7, 2.5, 2.8, 2.5, 0.5, -1.5]
    assert res1["raw_correction_factors_db"] == expected_raw, f"Correction values mismatch. Got {res1['raw_correction_factors_db']}, expected {expected_raw}"
    ...
    assert res2["bone_conduction_warning"] is True, "Test Case 2 failed: should detect bone conduction"
    print("✅ Test Case 2 Passed!")

    # Test case 3: Clipping limits (Safety check)
    print("\n--- Test Case 3: Safety Calibration Clipping ---")
    # We will simulate a very sensitive user headphone by setting up an imaginary model
    # which has extremely high response, forcing clipping.
    # We will temporarily modify the mock database to test this.
    from hearing_agent.retrieval import MOCK_HEADPHONES
    MOCK_HEADPHONES["ultra sensitive phones"] = {
        "source": "oratory1990",
        "form_factor": "over-ear",
        "frequencies": [250, 500, 1000, 2000, 4000, 8000],
        "smoothed": [25.0, 30.0, 28.0, 32.0, 25.0, 20.0], # high db
        "raw_url": "https://raw.githubusercontent.com/jaakkopasanen/AutoEq/master/results/oratory1990/over-ear/UltraSens/UltraSens.csv"
    }
    ...
    assert any(val == -15.0 for val in res3["correction_factors_db"]), "Correction factors should be clipped to MIN_CORRECTION_DB (-15.0)"
    assert res3["clipping_warning"] is not None, "Should generate a clipping warning"
